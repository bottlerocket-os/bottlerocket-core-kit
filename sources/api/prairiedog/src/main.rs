...
fn prepare_boot() -> Result<()> {
    ...
    if uki_image_from_env() {
        ...
        info!("GRUB image detected; using the boot partition of the active partition set");
        let state = layout.into_state().context(error::LoadStateSnafu)?;
...
fn uki_image_from_env() -> bool {
    match env::var(UKI_IMAGE_ENV) {
        Ok(value) if value.trim().eq_ignore_ascii_case("true") => {
            info!("{UKI_IMAGE_ENV} is set to '{value}' in the environment");
            true
        }
        Ok(value) => {
            warn!(
                "{UKI_IMAGE_ENV} is set to '{value}' in the environment, which is not a \
                 recognized true value; assuming this is not a UKI image"
            );
            false
        }
        Err(env::VarError::NotUnicode(value)) => {
            warn!(
                "{UKI_IMAGE_ENV} is set in the environment to a value that is not valid UTF-8 \
                 ({}); assuming this is not a UKI image",
