...
fn usage() -> ! {
    let msg = &format!(
        ...
        Subcommands:
            ...
            ephemeral-storage list-disks List the discovered ephemeral disks that can be initialized.
            ephemeral-storage list-dirs  List the directories that can be bound to ephemeral storage.
        ...
        ephemeral-storage init options:
            -fs, --filesystem          Filesystem to initialize the array as (ext4 or xfs). Default is
                                       xfs. If a single disk is provided, its mounted directly without
    ...
    while let Some(arg) = iter.next() {
        match arg.as_ref() {
            "-fs" | "--filesystem" => {
                match iter
                    .next()
                    .unwrap_or_else(|| usage_msg("Did not give argument to -fs | --filesystem"))
                    .as_str()
                {
            ...
            "--disks" => {
                let names = collect_non_args(&mut iter);
                if names.is_empty() {
...
fn parse_ephemeral_storage_bind_args(args: Vec<String>) -> EphemeralStorageSubcommand {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_ref() {
            "--dirs" => {
                targets = collect_non_args(&mut iter);
                if targets.is_empty() {
