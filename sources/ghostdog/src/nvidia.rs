...
//!   present, else `lts`.
//!
//! - **Flavour (role): `tesla`/`open-gpu`/`grid`** — [`match_flavour`] backs the
...
// files under `/usr/share/.nvidia/<branch>/...` (a sibling of the overlay targets, so
// overlayfs does not reject an overlapping lowerdir). The supported-devices file
...
lazy_static! {
    ...
    // subdevice may indicate the GRID flavour. Checked against NVIDIA_GRID_SUBDEVICES.
    ...
    // subsystem_device). A present GRID device with one of these uses the grid flavour.
    ...
    // (pb / r595) but are absent from the driver's `supported-gpus.json` -- i.e.
    // SR-IOV VF devids collected by querying instances. The open-gpu-supported
    // devices are resolved dynamically from each branch's shipped file (see
    // `requires_pb`); this set is only the curated supplement that file cannot cover.
    //
    // Cross-reference: a pb device that is also a GRID device (e.g. `2c3a`) appears
    // here (branch selection, `0x`-upper form) AND in NVIDIA_GRID_DEVICE_IDS (flavour
    // selection, raw-lower form). Keep the two in sync for new vGPU instance types.
...
enum NvidiaBranch {
    /// Long-Term Support branch (currently r580).
    Lts,
    /// Product Branch (currently r595).
    Pb,
}
...
impl NvidiaBranch {
    ...
    /// uppercase form). Empty if the file is absent (e.g. single-driver image)
    /// or unreadable.
    fn open_gpu_device_ids(&self) -> HashSet<String> {
...
/// Reports whether `driver_name` is the flavour preferred for the hardware
/// present.
pub(crate) fn match_flavour(driver_name: &str) -> Result<bool> {
    let active_branch = branch_from_marker()
        .map(|b| b.as_str())
        .unwrap_or("none (single-driver image)");
    let preferred_driver = find_preferred_driver()?;
    if driver_name == preferred_driver {
        log::info!(
            "match-nvidia-driver: selected flavour '{preferred_driver}' (branch {active_branch})"
        );
        Ok(true)
    } else {
        log::debug!(
            "match-nvidia-driver: branch={active_branch}, requested={driver_name}, \
             preferred={preferred_driver}, no match"
        );
        Ok(false)
    }
}

/// Determines which driver flavour (`grid`/`open-gpu`/`tesla`) to use based on the
...
fn find_preferred_driver() -> Result<String> {
    ...
    let mut subdevice_ids = present_devices
        ...
        .into_iter();
    log::debug!(
        "flavour detection: {} present NVIDIA device(s), {} open-gpu device(s) in supported file",
        present_devices.len(),
        open_gpu_device_set.len()
    );
    ...
    if subdevice_ids.any(|subdevice| NVIDIA_GRID_SUBDEVICES.contains(subdevice.as_str())) {
        log::debug!("flavour detection: matched a GRID subdevice");
        return Ok("grid".to_string());
    }

    if unique_ids.any(|input_device| open_gpu_device_set.contains(&input_device)) {
        log::debug!("flavour detection: a present device is open-gpu-supported");
        Ok("open-gpu".to_string())
    } else {
        log::debug!("flavour detection: no grid/open-gpu match, defaulting to tesla");
        Ok("tesla".to_string())
    }
}
...
///
/// On a multi-driver image `match-nvidia-branch` writes a marker for the chosen
/// branch and the module-load units that call us are ordered after it, so we read
/// that branch's file directly (via the marker) rather than depending on the
/// overlay that later presents one branch at the canonical path. If there is no
/// marker or branch file (single-driver image), fall back to the canonical path.
fn open_gpu_supported_devices_path() -> PathBuf {
    if let Some(branch) = branch_from_marker() {
        let branched = branch.open_gpu_supported_devices_file();
        if branched.exists() {
            log::debug!(
                "using branch '{branch}' supported-devices file: {}",
                branched.display()
            );
            return branched;
        }
    }
    log::debug!("using canonical supported-devices file: {OPEN_GPU_SUPPORTED_DEVICES_PATH}");
    OPEN_GPU_SUPPORTED_DEVICES_PATH.into()
}
...
pub(crate) fn match_branch() -> Result<()> {
    ...
    log::debug!("writing branch marker: {}", branch.marker_path().display());
    write_marker(branch)
}

/// Resolves the branch: an explicit `settings.kernel.drivers.nvidia.branch` pin wins;
...
/// Reads the `settings.kernel.drivers.nvidia.branch` pin from the Bottlerocket API.
/// Returns `None` if the setting is unset, empty, invalid, or the API is
/// unreachable, so the caller falls through to PCI detection.
fn branch_setting() -> Result<Option<NvidiaBranch>> {
    ...
    let uri = "/?prefix=settings.kernel.drivers.nvidia.branch".to_string();
    let value = match rt.block_on(apiclient::get::get_uri(constants::API_SOCKET, uri)) {
        Ok(value) => value,
        Err(e) => {
            log::warn!("Failed to read driver-branch setting from API, using PCI detection: {e}");
            return Ok(None);
        }
    };
