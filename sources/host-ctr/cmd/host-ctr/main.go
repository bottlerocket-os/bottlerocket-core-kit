...
func parseImageURISpecialRegions(input string) (ecr.ECRSpec, error) {
	...
	fipsSupportedRegionMapping := map[string]string{
		"us-east-1":     "",
		"us-east-2":     "",
		"us-west-1":     "",
		"us-west-2":     "",
	...
	if matches[2] == "-fips" {
		region := matches[3]
		_, isFips := fipsSupportedRegionMapping[region]
		if !isFips {
			return ecr.ECRSpec{}, fmt.Errorf("%s: %s", "invalid FIPS region in internal mapping", region)
		}
